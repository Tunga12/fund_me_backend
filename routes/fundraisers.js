...
router.get("/:id", async (req, res) => {
  ...
  const fund = await Fundraiser.findOne({
    ...
    .populate({
      ...
      populate: {
        path: "id",
        select: "hasRaised shareCount status userId",
  ...
  if (!fund)
    return res
      .status(404)
      .send("A fundraiser with the given ID was not found.");
...
router.post("/", auth, async (req, res) => {
  ...
  try {
    task.save("teammembers", team).save("fundraisers", fund).run();
  } catch (e) {
...
router.put("/invitation/:fid", auth, async (req, res) => {
  ...
  fund.teams.forEach((team) => {
    if (team.userId.toString() === req.user._id.toString()) {
      ...
      if (team.status !== "pending")
        return res
          .status(404)
          .send("This invitation has already been accepted or declined.");
    }
  });
  ...
  if (accepted) {
    await Fundraiser.updateOne(
      { _id: req.params.fid, "teams.userId": user._id },
      {
        $set: {
          "teams.$.status": "accepted",
        },
    ...
    try {
      task
        .update("teammembers", { _id: teamid }, { isDeleted: true })
        .update(
          "fundraisers",
          { _id: id },
          { $pull: { teams: { userId: user._id } } }
        )
        .run();
    } catch (e) {
      console.log(e.message);
...
router.put("/:id", auth, async (req, res) => {
  ...
  if (req.body.likeCount) {
    if (parseInt(fund.likeCount) != parseInt(req.body.likeCount)) {
      ...
      fund = await Fundraiser.findByIdAndUpdate(
        req.params.id,
  ...
  fund = await Fundraiser.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
  }).select("-isDeleted");
...
router.delete("/:id", auth, async (req, res) => {
  const fund = await Fundraiser.findByIdAndUpdate(
    req.params.id,
    { isDeleted: true },
    { new: true }
  );
...
// Get fundraisers number in date range (for admin)
