...
router.get("/:id", auth, async (req, res) => {
  ...
  if (!withdraw)
    return res
      .status(404)
      .send("A withdrawal with the given ID was not found.");
  res.send(withdraw);
});
...
router.post("/beneficiary/invitation/:fid", auth, async (req, res) => {
  ...
  const alink = `${config.get("link")}/accept?fundraiser=${
    req.params.fid
  ...
  const mailOption = {
    ...
    html:
      "Hello,<br> You are invited to withdraw the money raised on your behalf. You can <a href=" +
      alink +
      ">accept</a> or <a href=" +
      dlink +
      ">decline</a> the invitation.",
  };
  ...
  const newNot = new Notification({
    ...
    content:
      "Hello, You are invited to withdraw the money raised on your behalf.",
    target: fund._id,
  });
...
router.get("/invitation/accept/:fid", async (req, res) => {
  ...
  const newNot = new Notification({
    ...
    content:
      "Your beneficiary withdrawal request invitation has been accepted.",
    target: fund._id,
  });
...
router.get("/invitation/decline/:fid", async (req, res) => {
  ...
  const newNot = new Notification({
    ...
    title: `Withdrawal Request`,
    content:
      "Your beneficiary withdrawal request invitation has been declined.",
    target: fund._id,
  });
...
router.post("/", auth, async (req, res) => {
  ...
  if (req.body.isOrganizer === true) {
    if (fund.organizer.toString() != req.user._id.toString()) {
      return res
        .status(403)
        .send("You are not authorized to submit the withdrawal form.");
    }
  } else {
  ...
  const id = mongoose.Types.ObjectId(withdraw.fundraiser);

  const task = new Fawn.Task();
  try {
    task
      .save("withdraws", withdraw)
...
*/
  ...
  if (!fund)
    return res
      .status(404)
      .send("A fundraiser with this withdrawal id is not found");
  ...
  if (!accepted) {
    if (withdraw.status.toLowerCase() !== "declined") {
      if (!req.body.reason) {
        return res
          .status(400)
          .send("A reason is required when declining a withdrawal request.");
      } else {
        withdraw = await Withdraw.findByIdAndUpdate(id, {
          status: "declined",
      ...
      return res
        .status(400)
        .send("This withdrawal request has already been declined.");
    }
  } else {
    if (withdraw.status.toLowerCase() !== "accepted") {
      ...
      return res
        .status(400)
        .send("This withdrawal request has already been accepted.");
    }
  }
  const newNot = new Notification({
    notificationType: "Withdrawal",
...
router.delete("/:id", auth, async (req, res) => {
  ...
  try {
    task
      .update("withdraws", { _id: withdraw._id }, { isDeleted: true })
      .update(
        "fundraisers",
